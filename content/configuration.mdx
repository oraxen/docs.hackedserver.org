...
| `bedrock.toml` | Bedrock Edition player detection via Geyser/Floodgate |
...
| `<source>` | Detection source (e.g., "Geyser" or "Floodgate" for Bedrock detection) |
...
| `message_has` | string | No | Only trigger if message contains this text (case-insensitive) |
| `message_not_has` | string | No | Exclude if message contains this text (case-insensitive) |
| `category` | string | No | Grouping for display: `"loader"`, `"client"`, `"mod"`, or `"other"` (default) |
| `actions` | array | Yes | List of action IDs to execute |

<Callout type="info">
The `message_not_has` property is useful when two checks share the same channel and overlapping `message_has` values. For example, `forge` uses `message_not_has = "neoforge"` to avoid matching NeoForge clients.
</Callout>
...
<Callout type="info">
For detailed configuration including per-mod actions and advanced usage, see the dedicated [Lunar Client](/lunar-client) page.
</Callout>
...
<Callout type="info">
For detailed configuration including per-mod actions and example setups, see the dedicated [Forge/NeoForge](/forge-neoforge) page.
</Callout>

## Bedrock Detection (bedrock.toml)

HackedServer 3.15.0 adds Bedrock Edition player detection via Geyser and Floodgate APIs.

```toml
# Bedrock player detection via Geyser/Floodgate API
# Automatically detects which APIs are available
...
| `label` | string | `"Bedrock"` | Label for alerts (`<name>` placeholder) |
| `actions` | array | `[]` | Actions when Bedrock player detected |

<Callout type="info">
For detailed configuration including proxy setups and alternative detection methods, see the dedicated [Bedrock Detection](/bedrock) page.
</Callout>
