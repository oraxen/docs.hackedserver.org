...
message_not_has = "optional_exclusion"
category = "mod"
actions = ["alert"]
```
...
| `message_not_has` | No | Exclude if message contains this text (case-insensitive) |
| `category` | No | Grouping for display in `/hs inv`: `"loader"`, `"client"`, `"mod"`, or `"other"` (default) |
...
If two checks share the same channel and overlapping content, use `message_not_has` to prevent false positives:
...
name = "NeoForge"
channels = ["MC|Brand", "minecraft:brand"]
message_has = "neoforge"
category = "loader"
actions = ["alert"]
```

<Callout type="info">
Without `message_not_has`, a NeoForge client would trigger both the `forge` and `neoforge` checks since "neoforge" contains "forge".
</Callout>
...
3. **Use `message_not_has`** when two checks share overlapping content on the same channel
4. **Set a `category`** so checks are grouped correctly in the `/hs inv` GUI (`"loader"`, `"client"`, `"mod"`, or `"other"`)
5. **Test with debug mode** to verify your checks work correctly
6. **Start with `alert` action** before adding punishment actions
7. **Document your custom checks** with comments in the TOML file
